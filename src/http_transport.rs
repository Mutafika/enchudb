...
//! ```ignore
//! // サーバー側 (origin)
//! let relay = HttpRelay::start("127.0.0.1:8080").unwrap();
//! // relay はバックグラウンドスレッドで listen、drop で shutdown
//!
//! // クライアント側 (edge)
//! let t: Arc<dyn Transport> = Arc::new(HttpTransport::new("http://127.0.0.1:8080".to_string()));
//! let syncer = Syncer::new(eng, t);
//! syncer.pull_once(1);
//! ```
//!
//! # 注意
...
use crate::transport::{
    decode_batch, encode_batch, Transport, WireRecord,
};
...
impl HttpTransport {
    ...
    /// let hlc = client.bootstrap_to("/tmp/replica.db")?;
    /// let replica = Engine::open_replica("/tmp/replica.db")?;
    /// // 以降 Syncer で incremental sync
    /// ```
...
mod tests {
    ...
    fn publish_and_pull_roundtrip() {
        ...
        // wait a tiny bit for the request to land (async-ish)
