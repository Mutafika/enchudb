...
fn main() {
    ...
    for i in 0..100u64 {
        let eid = enchudb_oplog::make_eid(1, (10_000 + i) as u32);
    ...
    const BATCH_SIZE: u32 = 500;
    {
        ...
        for batch_start in (0..WARMUP_COUNT).step_by(BATCH_SIZE as usize) {
            ...
            for i in batch_start..end {
                let eid = enchudb_oplog::make_eid(1, WARMUP_BASE + i);
        ...
        // replica が追いつくまで待つ
        let t1 = Instant::now();
        while replica.get(enchudb_oplog::make_eid(1, WARMUP_BASE + WARMUP_COUNT - 1), "val").is_none() {
