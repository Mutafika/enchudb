//! Scope (旧 TenantView) invariant — issue #12。
//!
...
/// pattern A (container.scope("alice")) と pattern B (alice_db.as_scope()) が
...
fn invariant_holds_across_topologies() {
    ...
    {
        let mut alice = db_a.scope_mut("alice");
        alice.table("users")
            .number("id")
            .tag("name")
            .number("age")
    ...
    {
        // scope("alice") の view 経由で insert
        let alice = db_a.scope("alice");
    ...
    {
        let mut root = db_b.as_scope_mut();
        root.table("users")
            .number("id")
            .tag("name")
            .number("age")
...
/// container DB に alice と bob を両方入れて、 scope("alice").list_tables() が
...
fn list_tables_isolation_and_prefix_stripping() {
    ...
    {
        db.scope_mut("alice").table("users").number("id").primary_key("id").build().unwrap();
        db.scope_mut("alice").table("posts").number("id").primary_key("id").build().unwrap();
        db.scope_mut("bob").table("users").number("id").primary_key("id").build().unwrap();
    }

    // scope("alice") は alice のものだけ、 short name で返る
    let alice_tables: Vec<String> = db.scope("alice").list_tables()
    ...
    let bob_tables: Vec<String> = db.scope("bob").list_tables()
        .into_iter().map(|t| t.name).collect();
    assert_eq!(bob_tables, vec!["users".to_string()], "bob view sees only users");

    // root view は全部見える (raw 名で)
    let root_tables: Vec<String> = db.as_scope().list_tables()
        .into_iter().map(|t| t.name).collect();
...
/// build_via_scope_mut で建てたものが、 同 process 内で scope() の view から
/// 引ける + reopen 後にも scope() で引ける = persistence + 解決の round trip。
#[test]
fn build_via_scope_mut_round_trip() {
    let path = tmp_path("round_trip");
    cleanup(&path);

    {
        let mut db = Database::create(&path).unwrap();
        db.scope_mut("alice").table("memos")
            .number("id")
            .leaf("body")
            .primary_key("id")
            .build()
            .unwrap();
        let alice = db.scope("alice");
...
#[test]
fn interleaved_build_and_read_via_scope() {
    let path = tmp_path("interleaved");
    cleanup(&path);

    let mut db = Database::create(&path).unwrap();

    // 1) scope_mut で users 定義
    db.scope_mut("alice")
        .table("users")
        .number("id")
        .tag("name")
...
/// `as_scope()` (root view) が単独 DB の全 table を見せる (= pattern B のための
/// fallback、 prefix なし)。
#[test]
fn as_scope_sees_all_tables_unprefixed() {
    let path = tmp_path("root_view");
    cleanup(&path);

    let mut db = Database::create(&path).unwrap();
    {
        let mut root = db.as_scope_mut();
        root.table("items").number("id").primary_key("id").build().unwrap();
        root.table("notes").number("id").primary_key("id").build().unwrap();
    }

    let names: Vec<String> = db.as_scope().list_tables().into_iter().map(|t| t.name).collect();
    assert!(names.contains(&"items".to_string()));
    assert!(names.contains(&"notes".to_string()));
    assert_eq!(names.len(), 2);

    // root view で prefix 無しで table が引ける
    let items = db.as_scope().get_table("items").expect("items via root view");
