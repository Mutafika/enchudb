...
fn single_condition_routes_to_owner_peer() {
    ...
    assert_eq!(enchudb_oplog::eid_peer(r[0]), 1);
    assert_eq!(enchudb_oplog::eid_local(r[0]), enchudb_oplog::eid_local(e1));

    // size=10 は remote (B)
    let r = q_a.pull_raw("size", 10);
    assert_eq!(r.len(), 1, "remote B's size entity");
    assert_eq!(enchudb_oplog::eid_peer(r[0]), 2, "should be peer 2");
    assert_eq!(enchudb_oplog::eid_local(r[0]), enchudb_oplog::eid_local(e2));
...
fn explicit_router_falls_back_for_unknown_himo() {
    ...
    assert_eq!(r, Vec::<enchudb_oplog::EntityId>::new());
...
fn two_condition_query_crosses_shards() {
    ...
    assert_eq!(r.len(), 1);
    assert_eq!(enchudb_oplog::eid_peer(r[0]), 2);
...
fn replicated_entity_matches_across_shards_via_sync() {
    ...
    let shared_eid = enchudb_oplog::make_eid(0, shared_local);
