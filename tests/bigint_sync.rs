...
/// BigInt の主キー: 2 台が同じ主キーの row を別々に作ってから同期しても 1 row に束ねる (#141 の PK bind)。
#[test]
fn bigint_primary_keys_bind_across_peers() {
    let make = |path: &str, peer: u32| -> Arc<Database> {
        let mut b = Database::create_with_capacity(path, 65_536).unwrap();
        b.table("ids").bigint("id").tag("note").primary_key("id").build().unwrap();
        b.enable_sync().unwrap();
        let db = b.finish_with_oplog(16 * 1024 * 1024).unwrap();
        db.engine().set_peer_id(peer);
        db
    };
    let (pa, pb) = (tmp("pk_a"), tmp("pk_b"));
    let (db_a, db_b) = (make(&pa, 1), make(&pb, 2));
    let keys = [-(1i64 << 50), 0, 1_790_000_000_123, enchudb::schema::BIGINT_MAX];
    for k in keys {
        db_a.get_table("ids").unwrap().upsert().set("id", k).set("note", "a").commit().unwrap();
        db_b.get_table("ids").unwrap().upsert().set("id", k).set("note", "b").commit().unwrap();
    }
    let transport: Arc<dyn Transport> = Arc::new(InMemoryTransport::new());
    let sync_a = Syncer::new(db_a.arc_engine(), transport.clone());
    let sync_b = Syncer::new(db_b.arc_engine(), transport.clone());
    ...
    sync_a.pull_once(2);
    for db in [&db_a, &db_b] {
        let t = db.get_table("ids").unwrap();
        assert_eq!(t.all().count().unwrap(), keys.len(), "同じ主キーの row が束ねられていない");
        for k in keys {
            assert_eq!(t.where_eq("id", k).count().unwrap(), 1, "主キー {k} の row が 1 つでない");
        }
    }
    drop((sync_a, sync_b, db_a, db_b));
    cleanup(&pa);
    cleanup(&pb);
}
