...
- **`View` / `ViewMut`** ([issue #12](https://github.com/Mutafika/enchudb/issues/12)、 PR #13 + #14) — `enchudb-schema` に「物理 layout を隠す view layer」 を追加。 `Database::view(name)` / `view_mut(name)` で scope-prefixed view、 `as_view()` / `as_view_mut()` で root view を取り出す。 内部で table 名に `{name}.` prefix を自動付与する薄い wrapper、 storage layout は変えない。
