...
- **`set_peer_id` を sync 前に必ず呼ぶこと (foot-gun)**: resolve は `author_peer ==
  self.peer_id` を identity 判定に使う。 peer_id 未設定 (= 0) の node 同士が sync すると
  author 0 == self 0 で identity 扱いになり翻訳が無効化、 **#9 の衝突バグがそのまま再発する**
  (自分の entity をサイレント上書き)。 multi-peer では各 node に必ず非 0 の peer_id を設定する。
- **reopen 後の foreign LWW watermark は復元されない (削除済み entity 復活のリスク)**:
  `HlcStore` は in-memory で永続化されず、 reopen 時に local oplog を walk して再構築する。
  `gossip_remote_apply` off (default) では foreign data op が local oplog に載らないため、
  reopen 後は foreign entity の watermark (Delete tombstone を含む) が空になる。 これ自体は
  pre-#9 からの sync 設計上の制約だが、 #9 の永続 `.eidmap` により「同じ foreign が必ず同じ
  local slot に解決される」ようになったため、 **Delete より古い Tie が Delete 抜きで再配送
  されると、 削除済み entity が決定的に元の slot へ復活しうる**。 通常の full re-pull では
  Delete も再配送されて tombstone が張り直るので顕在化しないが、 partial subscription /
  ring reset / 第三 peer からの gossip fan-in では起こりうる。 → watermark (最低でも
  tombstone) の永続化を follow-up issue で対応予定。
- **配送順序による op ロス**: Content / Delete は確保先 table を導けないため
  `resolve_remote_eid_existing` (既存写像のみ) で引き、 未 map なら skip する。 ある entity の
  Tie と Content が **別々の pull** に分かれ Content が先に届くと、 skip された Content は
  pull cursor 前進で再試行されず失われうる (同一 commit / 同一 batch 内は HLC 順で Tie が
  先なので安全)。 table-less himo の Tie、 Content-only entity (一度も Tie されない) も同様に
  skip される。 厳密な retry (pending queue) は follow-up。 なお Untie は確保 (`resolve_remote_eid`)
  経由で local slot を anchor し untie HLC を記録するので、 out-of-order な古い Tie は LWW で
  正しく弾かれる (skip ではない)。
