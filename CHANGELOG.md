...
- **`set_peer_id` を sync 前に呼ぶこと**: resolve は `author_peer == self.peer_id` を
  identity 判定に使う。 peer_id 未設定 (= 0) のまま apply すると author 0 の record を
  self 扱いして翻訳しないため、 Syncer attach 前に peer_id を確定させる。
- **reopen 後の foreign LWW**: `HlcStore` は in-memory で、 `gossip_remote_apply` off
  では foreign data op が local oplog に載らない。 よって reopen 後は foreign entity の
  LWW watermark が空になる (= 旧挙動と同じ、 #9 が新規に壊すものではない)。 重複 replay
  は pull cursor (`last_pulled`) が防ぐ。 厳密な LWW の永続化は今後の課題。
- `.eidmap` に per-file CRC は無い (`.tables` と同方針)。 破損は parse 失敗 → 空 fallback
  + 再 sync で復旧。 両 sidecar への CRC 付与は将来のハードニング候補。
...
- Content-only foreign entity (= 一度も Tie されない entity の Content) は確保先 table を
  導けないので apply skip される (Delete-before-Tie と同じ扱い)。
