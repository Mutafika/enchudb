...
- **write は単一 consumer で直列**（append O(1) amortized、メモリ ~1 倍、write コスト不変）。
- 削除/更新は Cylinder を触らず（append-only）、read 側が **Column verify で stale を
  ...
  dup は read で dedup。compaction は後付け最適化（未実装）。
...
himo で over-count になる（append 数ベース、compaction まで）。append-only では従来通り正確。
