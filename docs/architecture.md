...
## 2. ValueType — value の意味付け

`define_himo(name, type, max_values_hint)` で 4 種類:

| ValueType | value 解釈 | dedupe | 典型用途 |
|---|---|---|---|
...
eng.define_himo_in("users", "name", ValueType::Tag, 0)?;
eng.define_himo_in("users", "age", ValueType::Number, 100)?;
...
- **用語集 (glossary)**: [`docs/glossary.md`](glossary.md) — layer 別の用語索引 (§1-§10) + §11 メンタルモデル (= card / deck / pile / table の物理メタファ階層) + §12 混同しやすい同形語 (table / tenant / view / schema / cylinder 等) の disambiguation
