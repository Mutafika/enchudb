...
| **tie / 紐 (動詞)** | engine | `(eid, himo, value)` の事実 1 個を engine に立てる操作。 Column と Cylinder を同期的に両方 update する。 metaphor では「1 枚のカードを置く」 (§11.1)。 同時に円柱の維持費 (= 前払い、 §11.2) を支払う |
...
**カード 1 枚** を立てる操作 (表 = 紐名、 裏 = 値)。 宇宙全体は 3 レジスタで書ける:

| レジスタ | 中身 | 実在性 |
|---|---|---|
| **事実** | カード 1 枚 = 1 tie = 3D 空間の 1 点 | 唯一の情報。 1 枚ずつしか無い (§11.1) |
| **予約** (= schema 族) | 「将来何をどう観測するか」 の事前宣言 | カードを 1 枚も保管しない (§11.2) |
| **ビュー** (= 観測) | 参照でその場で組む束 | 仮想。 観測時に生まれ、 保存されない (§11.3) |

予約とビューは対立概念ではなく、 **同じクエリパターンの 2 つの支払い方** —
前払い (= 予約、 write/create 時に固定費) と都度払い (= ビュー、 観測時に組む)。
どのパターンを前払いするかはクエリ (= 観測の頻度と形) が決める。

「持ってない」 = カード不在 (NULL 値ではない、 §12.8)。 全体像のメタファは
**モンジャラ** (= 蔓 = 紐、 entity = 蔓の交差点に隠れた本体)。
