...
| **value (u32)** | engine | tie 1 個の右辺 (= カード裏面の回答)。 解釈は `ValueType` 依存 (Number は値そのまま、 Tag/Leaf は vid、 Ref は target eid) |
...
### ValueType (= value の格納方式、 旧 HimoType — 0.10.0 で rename 実施)

旧名 `HimoType` は名前と実態が乖離していた (= 「Himo の型」 を declare してるように見えるが、 実は「value = カード裏面の格納方式」 を選んでる) ため、 **0.10.0 で `ValueType` に rename** (`himo_type()` accessor → `value_type()` も連動)。 variant 見直しの経緯と結論は §12.10。
...
`ValueType::Tag` / `ValueType::Leaf` (旧 HimoType::*) は実態と名前が乖離してる:
...
- `HimoType` → **`ValueType`**: **0.10.0 で実施済み (2026-07-06)** (= 名前と実態の乖離解消、 「value の格納方式」 を直接表現)
...
> 残る有効案は **`HimoType → ValueType` のみ** → **同日 0.10.0 で実施済み**
> (Scope rename と同じ breaking minor に同乗)。
