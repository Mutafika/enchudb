...
## 紐のメンタルモデル
- **紐にぶら下げる** = entity を紐に接続する（tie）。`tie(猫, "足", 4)` = "足"という紐に猫をぶら下げる（値=4）
- **紐を引く** = ぶら下がってるもの全部出す（pull_raw/query）。`pull_raw("足", 4)` = "足"を値4で引く → 猫,犬
- **円柱** = ある entity からぶら下がってる紐を全部束ねたもの。猫の円柱 = [足|4], [読み|ネコ], [分類|哺乳類]...
- **暗記カードの束** = 円柱の別表現。1枚のカード = 1本の紐（表=紐名、裏=値）。束 = 円柱
